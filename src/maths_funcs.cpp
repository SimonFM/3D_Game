...
mat4 perspective(float fovy, float aspect, float nearVar, float farVar) {
	float fov_rad = fovy * ONE_DEG_IN_RAD;
	float range = tan(fov_rad / 2.0f) * nearVar;
	float sx = (2.0f * nearVar) / (range * aspect + range * aspect);
	float sy = nearVar / range;
	float sz = -(farVar + nearVar) / (farVar - nearVar);
	float pz = -(2.0f * farVar * nearVar) / (farVar - nearVar);
...
mat4 ortho(float left, float right, float bottom, float top, float nearVar, float farVar) {
	...
	//Diagonals
	result.m[0] = 1.5f / (right - left);
	result.m[5] = 1.5f / (top - bottom);
	result.m[10] = 1.5f / (farVar - nearVar);
	//Verticals
	result.m[3] = -1.0f * ((left + right) / (right - left));
	result.m[7] = -1.0f * ((top + bottom) / (top - bottom));
	result.m[11] = -1.0f * ((farVar + nearVar) / (farVar - nearVar));
